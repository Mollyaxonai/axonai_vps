...
from opencap_core.utils import importMetadata, loadCameraParameters, getVideoExtension
from opencap_core.utils import getDataDirectory, getOpenPoseDirectory, getMMposeDirectory
from opencap_core.utilsAugmenter import augmentTRC
from opencap_core.utilsChecker import saveCameraParameters
from opencap_core.utilsChecker import calcExtrinsicsFromVideo
from opencap_core.utilsChecker import isCheckerboardUpsideDown
from opencap_core.utilsChecker import autoSelectExtrinsicSolution
from opencap_core.utilsChecker import triangulateMultiviewVideo
from opencap_core.utilsChecker import writeTRCfrom3DKeypoints
from opencap_core.utilsChecker import popNeutralPoseImages
from opencap_core.utilsChecker import rotateIntrinsics
from opencap_core.utilsSync import synchronizeVideos
from opencap_core.utilsDetector  import runPoseDetector
# from utilsAugmenter import augmentTRC
from opencap_core.utilsOpenSim import runScaleTool, getTRCTimeRange, getScaleTimeRange, runIKTool, generateVisualizerJson
from opencap_core.defaults import DEFAULT_SYNC_VER
...
def main(sessionName="Trial_Session", trialName = "33", trial_id="33", cameras_to_use=['all'],
    ...
    if runOpenSimPipeline:
        ...
        if scaleModel:
            ...
            # Path TRC file (neutral pose trial).
            pathTRCFile4Scaling = pathAugmentedOutputFiles[trialName]
            # Use full time range of neutral-pose TRC; no static-phase detection needed.
            timeRange4Scaling = getTRCTimeRange(pathTRCFile4Scaling)
            logging.info('Scaling using neutral pose TRC time range [%.2f, %.2f] s',
                         timeRange4Scaling[0], timeRange4Scaling[1])
            try:
                # Run scale tool.
